"""
Modified from Dola and ICD Code
"""
import argparse
import time
import csv
...
class InterpolatedLayer(nn.Module):
    # categories: spherical_interpolation, log_space_interpolation, bezier_interpolation.
    def __init__(self, layer1, layer2, alpha, dropout_prob=0.1):
        super().__init__()
        self.alpha = alpha
        self.layer1 = layer1
        self.layer2 = layer2
        ...
        # new weight
        self.interpolated_params = self._create_interpolated_weights()

    def spherical_interpolation(self, p1: torch.Tensor, p2: torch.Tensor, alpha: float):
        
        #p1_log = torch.log1p(p1)
        #p2_log = torch.log1p(p2)
        #p1_norm = p1_log / torch.norm(p1, dim=-1, keepdim=True)
        #p2_norm = p2_log / torch.norm(p2, dim=-1, keepdim=True)
        p1_norm = p1 / torch.norm(p1, dim=-1, keepdim=True)
        p2_norm = p2 / torch.norm(p2, dim=-1, keepdim=True)

        # compute theta
        dot_product = torch.sum(p1_norm * p2_norm, dim=-1, keepdim=True).clamp(-1.0, 1.0)
        ...
        # compute slerp
        slerp = (torch.sin((1 - alpha) * theta) / sin_theta) * p1 + (torch.sin(alpha * theta) / sin_theta) * p2
        
        return slerp
    
    def log_space_interpolation(self, p1: torch.Tensor, p2: torch.Tensor, alpha: float):

        p1_log = torch.log1p(p1)
        p2_log = torch.log1p(p2)
        interpolated = (1 - alpha) * p1_log + alpha * p2_log
        return torch.expm1(interpolated)     
    
    def bezier_interpolation(self, p1, p2, alpha):

        control_point = (p1 + p2) / 2  
        return (1 - alpha) ** 2 * p1 + 2 * (1 - alpha) * alpha * control_point + alpha ** 2 * p2
    
    def _create_interpolated_weights(self):
        
        interpolated_params = {}
        for name, param1 in self.layer1.named_parameters():
            if name in dict(self.layer2.named_parameters()):
                param2 = dict(self.layer2.named_parameters())[name]
                interpolated_params[name] = self.spherical_interpolation(param1.data, param2.data, self.alpha)
            else:
                raise ValueError(f"params are not match")
        
        for name, buffer1 in self.layer1.named_buffers():
            if name in dict(self.layer2.named_buffers()):
                buffer2 = dict(self.layer2.named_buffers())[name]
                interpolated_params[name] = self.spherical_interpolation(buffer1.data, buffer2.data, self.alpha)
            else:
                raise ValueError(f"buffers are not match")
        return interpolated_params
    

    def forward(self, *args, **kwargs):

        original_params = {name: param.clone() for name, param in self.layer1.named_parameters()}
        for name, param in self.layer1.named_parameters():
            param.data.copy_(self.interpolated_params[name])

        output = self.layer1(*args, **kwargs)
...
def extend_model_with_interpolation(model, layer_index, alpha):
    ...
    assert layer_index < len(transformer_layers) - 1, "Invalid layer_index!"

    layer1 = transformer_layers[layer_index]
    layer2 = transformer_layers[layer_index + 1]

    interpolated_layer = InterpolatedLayer(layer1, layer2, alpha)

    new_layers = nn.ModuleList()
    for i, layer in enumerate(transformer_layers):
        new_layers.append(layer)
        if i == layer_index:
            new_layers.append(interpolated_layer)
...
class PLI:
    ...
    def __init__(self, model_name, device="cuda", max_gpu_memory=39, amateur_model_name=None, num_gpus=-1, amateur_model_nums_gpus=-1):
        ...
        # Conduct Premature Layers Interpolation
